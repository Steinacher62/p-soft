<%@ Control Language="c#" AutoEventWireup="True" Codebehind="ProjectTreeCtrl.ascx.cs" Inherits="ch.appl.psoft.Project.Controls.ProjectTreeCtrl" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
<script  type='text/javascript'>
	...
	function move(sourceType, sourceId, targetId)
	{
		if (sourceType == "TreeID") {
			wsMoveRow("PARENT_ID", targetId, "PROJECT", sourceId);
			location.reload(true);
		}
		else if (sourceType == "PhaseID") {
			wsMoveRow("PROJECT_ID", targetId, "PHASE", sourceId);
		}
		else return false;
	...
	{
		if (sourceType == "TreeID") {
			wsCopyRow("PROJECT", targetId, "PROJECT", sourceId, true);
		}
		else if (sourceType == "PhaseID") {
			wsCopyRow("PROJECT", targetId, "PHASE", sourceId, true);
		}
		else return null;
		location.reload(true);
		return null;
