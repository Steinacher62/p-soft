...
function setDataType(inValue) {
    ...
    var isDate = parseDate(inValue);
    if ((isDate == "NaN") || (isDate == null)) {
        if (isNaN(inValue)){
...
//-----------------------------------------------------------------------
function sortTable(aTableId, col, rowsPerPage){

    var i;
    tbl = document.getElementById(aTableId);
    if (tbl == null) 
      return;
    ...
    lastSort = col
    allTR = tbl.childNodes[0].childNodes
    // allTR now holds all the rows in the dataTable
    totalRows = allTR.length;
    if ((rowsPerPage > 0) && (totalRows > rowsPerPage + 1))
    {
...
function dateOrder(a,b){
    absOrder ? rVal = compareDates(a, parseDateFormat(a), b, parseDateFormat(b)) : rVal = compareDates(b, parseDateFormat(b), a, parseDateFormat(a))
    return rVal
}
//-----------------------------------------------------------------------
