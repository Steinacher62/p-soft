namespace ch.appl.psoft.MbO.Controls
{
    ...
    public partial  class ListView : PSOFTSearchListUserControl {
        ...
        {
            get { return Global.Config.baseURL + "/MbO/Controls/ListView.ascx"; }
        }
            ...
            try
            {
                switch (_context)
                {
                    ...
                    case Objective.PERSON:
                        ...
                        if ((Request.QueryString["id"] != null && ! Request.QueryString["id"].Equals("0")) && (Request.QueryString["turnid"] == "" || Request.QueryString["turnid"] == null))
                        {
                            turnid = _db.lookup("OBJECTIVE_TURN_ID", "OBJECTIVE", "ID = " + Request.QueryString["id"].ToString().Replace("0,","")).ToString();
                        }
        ...
        private void loadTable()
        {
            ...
            _table.Columns["NUMBER"].ExtendedProperties["OrdNum"] = 1;
            _table.Columns["TITLE"].ExtendedProperties["OrdNum"] = 2;
            _table.Columns["STARTDATE"].ExtendedProperties["OrdNum"] = 3;
        ...
        private void applyClick(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (TableRow r in listTab.Rows)
                {
                    foreach (TableCell c in r.Cells)
                    {
                        ...
                        switch (ids[0])
                        {
                            ...
                            case "RATING_WEIGHT":
                            case "RATING_DATE":
                                if (c.Controls[0].GetType().Name == "RadNumericTextBox"&& sender == rating)
                                {
                                    ...
                                    if (valStr != "null")
                                    {
                                        try
                                        {
                                            ...
                                            double maxValue = 100;
                                            if(tb.ID.ToString().StartsWith("RATING-"))
                                            {
                                                maxValue = double.Parse(Global.Config.getModuleParam("mbo", "maxRatingVal", "0"));
                                            }
                                            if (val < 0.0 || val > maxValue)
                                            {
                ...
                {
                    // get turn id from querystring, if not set, use default
                    turnid = _db.Objective.turnId.ToString();
                    if (Request.QueryString["turnid"] != null && Request.QueryString["turnid"] != "")
                    {
                        turnid = Request.QueryString["turnid"];
                    }
