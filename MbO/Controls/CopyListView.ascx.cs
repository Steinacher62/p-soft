namespace ch.appl.psoft.MbO.Controls
{
    ...
    /// </summary>
    public partial  class CopyListView : PSOFTSearchListUserControl {
        ...
        {
            get { return Global.Config.baseURL + "/MbO/Controls/CopyListView.ascx"; }
        }
        ...
        private void loadList()
        {
            ...
            InfoBoxEnabled = true;
            HeaderEnabled = true;
            string url = Global.Config.baseURL + "/MbO/Detail.aspx?context=" + _context + "&contextId=" + _contextId + "&id=%ID&view=%V&orderColumn=" + base.OrderColumn + "&orderDir=" + base.OrderDir;
            ...
            try
            {
                switch (_context)
                {
                    case Objective.SEARCH:
                        //DetailEnabled = false;
                        url = Global.Config.baseURL + "/MbO/Detail.aspx?id=%ID&view=%V&orderColumn=" + base.OrderColumn + "&orderDir=" + base.OrderDir;
                        DeleteEnabled = false;
                        EditEnabled = false;
                    ...
                    case Objective.PERSON:
                        ...
                        if (Request.QueryString["id"] != null && (Request.QueryString["turnid"] == "" || Request.QueryString["turnid"] == null))
                        {
                            turnid = _db.lookup("OBJECTIVE_TURN_ID", "OBJECTIVE", "ID = " + Request.QueryString["id"].ToString()).ToString();
                        }
                ...
                _sql += " order by TYP";  // TYP," + OrderColumn + " " + OrderDir;
        ...
        private void applyClick(object sender, System.EventArgs e)
        {
            ...
            //            case "RATING_WEIGHT":
            //            case "RATING_DATE":
            //                if (c.Controls[0] is TextBox && sender == rating) {
            ...
            //                +" and o.objective_turn_id = "+_db.Objective.turnId
            //                +" and o.typ in ("+_db.Objective.objectiveFilter+")";
            //            _db.execute(sql);  
            //        }
            //    }
        ...
        private void nextClick(object sender, System.EventArgs e)
        {
            ...
            foreach (TableRow r in listTab.Rows)
            {
                ...
                if (r.Cells[0].Controls.Count > 0 && r.Cells[0].Controls[0] is CheckBox)
                {
                    CheckBox chk = (CheckBox)r.Cells[0].Controls[0];

                    if (chk.Checked)
                    {
                        string ids = r.Cells[0].Controls[0].ID;
                        string id = ids.Split(new char[] { '_' })[1];
                        objectiveIds.Add(id);
                    }
                }
            }
            ...
            //copy objectives
            _db.connect();
            string turnId = _db.lookup("WERT", "PROPERTY", "TITLE = 'turn'").ToString();
            string orgentityId = _db.lookup("ORGENTITY.ID", "ORGENTITY INNER JOIN JOB ON ORGENTITY.ID = JOB.ORGENTITY_ID INNER JOIN EMPLOYMENT ON JOB.EMPLOYMENT_ID = EMPLOYMENT.ID INNER JOIN PERSON ON EMPLOYMENT.PERSON_ID = dbo.PERSON.ID", "(JOB.HAUPTFUNKTION = 1) AND (PERSON.ID = " + personID.ToString() + ")", true);
            string templateSQL = "INSERT INTO OBJECTIVE (PARENT_ID, NUMBER, TITLE, ORGENTITY_ID, PERSON_ID, DESCRIPTION, OBJECTIVE_TURN_ID, MEASUREMENT_TYPE_ID, CURRENTVALUE, VALUEIMPLICIT, TARGETVALUE, STARTDATE, DATEOFREACHING, CURRENTDATE, ARGUMENT, BSC, STATE, STATEDATE, TYP, FLAG, ACTIONNEED, MEASUREKRIT) SELECT "
                               + "PARENT_ID, NUMBER, TITLE, '" + orgentityId + "' AS ORGENTITY_ID, '" + personID + "' AS PERSON_ID, DESCRIPTION, '" + turnId + "' AS OBJECTIVE_TURN_ID, MEASUREMENT_TYPE_ID, CURRENTVALUE, VALUEIMPLICIT, TARGETVALUE, STARTDATE, DATEOFREACHING, CURRENTDATE, ARGUMENT, BSC, STATE, STATEDATE, TYP, FLAG, ACTIONNEED, MEASUREKRIT "
                               + "FROM OBJECTIVE where ID=";

            foreach (string objectiveId in objectiveIds)
            {
                _db.execute(templateSQL + objectiveId);
            }
            _db.disconnect();
            ...
            string referer = Session["CopyObjectiveRef"].ToString();
            if (referer != "")
            {
                Response.Redirect(referer, true);
            }
        }
