...
namespace ch.appl.psoft.Tasklist
{
    ...
    /// </summary>
    public class PDFList : ch.appl.psoft.Report.PDFList {
        ...
        {           
            try 
            {
                ...
                if (!_append) 
                {
                    ...
                    _writer.WriteAttributeString("file-name","ampelGrau.gif");
                    _writer.WriteAttributeString("image-name","ampelGrau");
                    _writer.WriteEndElement();
                    _writer.WriteStartElement("image");
                    _writer.WriteAttributeString("directory",imagePath);
                    _writer.WriteAttributeString("file-name","ampelGruen.gif");
                    _writer.WriteAttributeString("image-name","ampelGruen");
                    _writer.WriteEndElement();
                    _writer.WriteStartElement("image");
                    _writer.WriteAttributeString("directory",imagePath);
                    _writer.WriteAttributeString("file-name","ampelOrange.gif");
                    _writer.WriteAttributeString("image-name","ampelOrange");
                    _writer.WriteEndElement();
                    _writer.WriteStartElement("image");
                    _writer.WriteAttributeString("directory",imagePath);
                    _writer.WriteAttributeString("file-name","ampelRot.gif");
                    _writer.WriteAttributeString("image-name","ampelRot");
                    _writer.WriteEndElement();
                    _writer.WriteEndElement();
        ...
        private string writeColumnHeader() 
        {
            ...
            foreach (DataRow row in _column.Rows) 
            {
                ...
                if (!DBColumn.IsNull(row["headername_mnemo"])) 
                {
                    ...
                    if (row["headername_mnemo"].ToString().EndsWith("ColumnState"))
                    {
                        _writer.WriteString(_map.get("reportLayout", ""));
                    }
                    else 
                    {
                ...
                foreach (DataRow cell in _column.Rows) 
                {
                    ...
                    foreach (string attr in cell[_db.langAttrName("reportcolumn", "attributname")].ToString().Split(',')) 
                    {
                        if (!DBColumn.IsNull(row[attr])) 
                        {
                            ...
                            if(_data.Columns[attr].ColumnName.ToLower().Equals("state"))
                            {
                                text = "";
                                _writer.WriteAttributeString("fill-color",colors[0]);
                                string className = "";
                                switch (display)
                                {
                                    case "0":
                                ...
                                switch (_db.Measure.getSemaphore(DBColumn.GetValid(row["id"],-1L), _db.Tasklist.getCriticalDays(DBColumn.GetValid(row["parent"], -1L))))
                                {
                                    case 0:
                                        className = "ampelRot";
                                        break;
                                ...
                                string a = cell["id"].ToString();
                                _writer.WriteStartElement("show-image");
                                _writer.WriteAttributeString("image-name", className);
                                _writer.WriteEndElement();
