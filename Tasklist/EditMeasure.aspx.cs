...
namespace ch.appl.psoft.Tasklist
{
    ...
    /// </summary>
    public partial class EditMeasure : PsoftEditPage
	{
		private const string PAGE_URL = "/Tasklist/EditMeasure.aspx";

		static EditMeasure()
		{
			SetPageParams(PAGE_URL, "backURL", "ID", "assignPerson", "hideTasklist");
		}
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            try
            {
                template = db.lookup("TEMPLATE", "MEASURE", "ID=" + id, false) == "1";
            }
            catch(Exception ex) 
            {
            ...
            this.BreadcrumbCaption = _mapper.get("tasklist","editmeasure");;

			// Setting main page layout
			PageLayoutControl = (PsoftPageLayout)this.LoadPSOFTControl(PsoftPageLayout.Path, "_pl");
            (PageLayoutControl as PsoftPageLayout).PageTitle = _mapper.get("tasklist","editmeasure");
			...
			// Setting parameters
			PendenzenMeasureEdit _detail = (PendenzenMeasureEdit)this.LoadPSOFTControl(PendenzenMeasureEdit.Path, "_detail");
            _detail.Template = template;
            _detail.BackUrl = GetQueryValue("backURL","");
			_detail.MeasureID = id;
