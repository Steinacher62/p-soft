...
namespace ch.appl.psoft.Performance
{
    public partial class SaveAllPerformanceratigs : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            _db.connect();
            DataTable ratings = _db.getDataTable("SELECT PERFORMANCERATING.ID AS RATING_ID, EMPLOYMENT.ID AS EMPLOYMENT_ID, PERFORMANCERATING.RATING_DATE, PERSON.ID, PERSON.PERSONNELNUMBER "
                                                + "FROM PERSON INNER JOIN EMPLOYMENT ON PERSON.ID = EMPLOYMENT.PERSON_ID "
                                                + "INNER JOIN PERFORMANCERATING ON EMPLOYMENT.ID = PERFORMANCERATING.EMPLOYMENT_REF "
                                                + "WHERE NOT PERSON.PERSONNELNUMBER IS null ORDER BY PERSON.PERSONNELNUMBER");
            foreach (DataRow rating in ratings.Rows)
            {
                try
                {
                    _name = _db.lookup("PNAME", "PERSON", "ID = " + rating["ID"].ToString()).ToString();
                    _firstname = _db.lookup("FIRSTNAME", "PERSON", "ID = " + rating["ID"].ToString()).ToString();
                    _performanceRatingID = Convert.ToInt32(rating["RATING_ID"]);
                    _employmentID = Convert.ToInt32(rating["EMPLOYMENT_ID"]);
                    _personID = Convert.ToInt32(rating["ID"]);
                    _personnelnumber = Convert.ToInt32(rating["PERSONNELNUMBER"]);
                    _ratingDat = rating["RATING_DATE"].ToString().Remove(10, 9);
                    //string redirectUrl = "http://" + Request.Url.Host + (Request.Url.IsDefaultPort ? "" : ":" + Request.Url.Port) + Global.Config.baseURL + "/Performance/EmploymentRating.aspx?performanceRatingID=" + ratingId + "&employmentID=" + employmentId + "&mode=report&personnelnumber=" +personnelNumber+"&save=true";
        ...
        private void export()
        {
            ...
            fileName = _name + "_" + _firstname + "_" + _personnelnumber + "_" + _ratingDat;
            ...
            if (reportClassName == "ch.appl.psoft.Performance.AveragePerformanceReportRectangle")
            {
                ...
                string rectangle_filename = fileName + ".pdf";
        ...
        private void SetPerformanceDiagramm(ReportDocument rpt1, DBData db)
        {
            ...
            if (Global.Config.getModuleParam("performance", "showLine", "1") == "1")
            {
                ...
                if (Global.isModuleEnabled("frauenfeld") || Global.isModuleEnabled("kreuzlingen"))
                {
