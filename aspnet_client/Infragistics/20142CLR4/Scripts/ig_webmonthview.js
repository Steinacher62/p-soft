...
function ig_CreateWebMonthView(props)
{
    ...
    if(!ig_DayOrientedView.prototype.isPrototypeOf(ig_WebMonthView.prototype))
    {
		...
		ig_WebMonthView.prototype._onClick = function(src, evt) 
		{
			...
			if(next || attr == "PREV")
			{
				if(!this.fireEvent(next ? "navigatenext" : "navigateprevious", evt, next ? 1: -1))	
				{
					...
					// N.A. 02/11/2014 Bug #158191 Fix the next and prev button to navigate to the next and prev months.
					// Active day becomes the first day of the new month.
					var firstDayOfNewMonth;
					if (next) {
						firstDayOfNewMonth = new Date(this.getCurrentMonth().getFullYear(), this.getCurrentMonth().getMonth() + 1, 1);
					} else {
						firstDayOfNewMonth = new Date(this.getCurrentMonth().getFullYear(), this.getCurrentMonth().getMonth() - 1, 1);
					}
					firstDayOfNewMonth.setMilliseconds(0);
					firstDayOfNewMonth.setSeconds(0);
					firstDayOfNewMonth.setMinutes(0);
					firstDayOfNewMonth.setHours(0);
					this._firstDayOfNewMonth = firstDayOfNewMonth;
					var activeDay = info.getActiveDay();
					activeDay.setMilliseconds(0);
					activeDay.setSeconds(0);
					activeDay.setMinutes(0);
					activeDay.setHours(0);
					var daysToMove = (firstDayOfNewMonth.getTime() - activeDay.getTime()) / (24 * 60 * 60 * 1000);
					daysToMove = Math.ceil(daysToMove);
					var lastVisibleDay = new Date();
					lastVisibleDay.setTime(this.getFirstVisibleDay().getTime() + 42 * 24 * 60 * 60 * 1000);
					if (next && (firstDayOfNewMonth < lastVisibleDay)) {
						this._additionalRefresh = true;
					}
					this.setCurrentMonth(firstDayOfNewMonth);
		...
		ig_WebMonthView.prototype.callbackRender = function(response, context)
		{
			...
			// N.A. 02/20/2014 Bug #158191: Add additianl navigation. 
			// This is needed when from example we are on 1st of March and then we click the next month.
			// The date is changed to 1st of April but the month is not chaged. This is becasue there is some problem and the server-side when the new month is calclated.
			// And the month is not changed on purpose, not to break functioanlity with appointments. So this is kind of workaround, but at least the change is minimilized,
			// and this additional navigate is done only in a few cases.
			var info = this.getWebScheduleInfo();
			if (this._additionalRefresh) {
				this._additionalRefresh = false;
				info.fireServerEvent("Navigate", "ActiveDay");
			}
			if (this._firstDayOfNewMonth) {
				info.setActiveDay(this._firstDayOfNewMonth, false, this._clientID);
				this._firstDayOfNewMonth = null;
			}
        }
