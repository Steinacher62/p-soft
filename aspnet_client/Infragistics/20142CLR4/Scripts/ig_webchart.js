...
function IGBrowser() 
{
	...
	this.IsIE				= (document.all ? true : false) || this.AgentName.indexOf("trident") != -1;
	...
	this.IsIE11Plus         = this.AgentName.indexOf("rv:") != -1 && parseFloat(this.AgentName.substr(this.AgentName.indexOf("rv:") + 3)) >= 11;
	this.IsIEPlus           = this.IsIE9Plus || this.IsIE11Plus;
	...
	{ 
		if ((this.IsFirefoxOrSafari || IGB.IsIEPlus) && document.documentElement) h += "px";
		if (this.IsNetscape) 
		{
	...
	{
	    if ((this.IsFirefoxOrSafari || IGB.IsIEPlus) && document.documentElement) w += "px";
		if (this.IsNetscape) 
		{
	...
	{
	    if ((this.IsFirefoxOrSafari || IGB.IsIEPlus) && document.documentElement) x += "px";
	...
	{
	    if ((this.IsFirefoxOrSafari || IGB.IsIEPlus) && document.documentElement) y += "px";
...
function IGWindowViewer(id, imageId, vuid, srcBounds, destBounds) 
{
	...
	this.Render=function()
	{
		...
		if ((IGB.IsFirefoxOrSafari || IGB.IsIEPlus) && document.documentElement)
		{
			w += "px";
			h += "px";
		}
...
function IGUltraChart(id, imageUrl, uniqueId)
{
	...
	this.onallevent=function(evt, id, args)
	{
		...
		if (evt.type == "mousemove" && (IGB.IsNetscape6 || IGB.IsMac || IGB.IsOpera || IGB.IsSafari || IGB.IsIEPlus))
		{
