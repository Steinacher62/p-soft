...
//vs 12/06/13
var iged_obj=null,iged_ie=null;
...
function iged_create(tag)
{
	var doc=opener.iged_all._cur;
	if(doc)doc=doc._cDoc?doc._cDoc():(doc._doc?doc._doc():null);
	if(!doc)doc=document;
...
function iged_isIE()
{
	if(iged_ie==null)
	{
		iged_ie=iged_el("isIE");
		iged_ie=iged_ie?iged_ie.value=="true":!!document.all;
	}
	return iged_ie;
}
...
function iged_insImg()
{
	...
	if(iged_update)
	{
		if(iged_isIE())opener.iged_doImgUpdate(txt);
		else opener.iged_doImgUpdate(img);
	}
	else
	{
		if(iged_isIE())opener.iged_insText(txt,false,false);
		else
		{
...
{
	var v=iged_el("flashSource"),fl=iged_create("EMBED");
	if(!src)src=v.value;
	fl.src=iged_src=src;
...
function iged_insFlash()
{
	...
	if(iged_obj)
	{
		if(iged_isIE())opener.iged_insText(iged_obj.outerHTML,false,false);
...
{
	var v=iged_el("mediaSource"),md=iged_create("EMBED");
	if(!src)src=v.value;
	md.src=iged_src=src;
