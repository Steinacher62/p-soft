...
namespace ch.appl.psoft.SPZ
{
    ...
    public partial class Detail : PsoftTreeViewPage {
        ...
        protected override void Initialize() {
            ...
            base.SubNavMenuUrl = "/SPZ/SubNavMenu.aspx?context="+_context+"&contextId="+_contextId+"','addObjective','"+_id;
        }
            ...
            try	{
                ...
                if (_view == "trace") {
                    ...
                    if (_view == "detail") {
                        ...
                        {
                            if (turn_link.IndexOf("&id=") > -1)
                            {
                                turn_link = turn_link.Substring(0, turn_link.IndexOf("&id="));
                            }
                        ...
                        if (_id == 0)
                        {
                            links.LinkGroup1.AddLink(_mapper.get("mbo", "objective"), _mapper.get("mbo", "copyObjectiveFrom"), "/SPZ/CopyObjective.aspx?contextId=" + _contextId);
                        }
                        ...
                        if (_id > 0) {
                            ...
                            if (_typ != Objective.JOB_TYP && _typ != Objective.PERSON_TYP && _typ != Objective.PROJECT_TYP && !_db.Objective.isPersonFilterOnly) {
                                ...
                                if(true) {
                                    ...
                                    case Objective.State.DELETE:
                                        links.LinkGroup1.AddLink(_mapper.get("mbo", "objective"),	_mapper.get("mbo", "newObjective"),	"/SPZ/Detail.aspx?typ=0&view=add&id="+ _id);
                                        links.LinkGroup1.AddLink(_mapper.get("mbo", "objective"),	_mapper.get("mbo", "copyObjective"), "/SPZ/Detail.aspx?typ=0&view=copy&id=" +_id);
                                        links.LinkGroup1.AddLink(_mapper.get("mbo", "objective"),	_mapper.get("mbo", "linkOE"),	"/SPZ/Detail.aspx?view=link&context="+_context+"&contextId="+_contextId+"&id=" + _id);
                                        break;
                                    }
                                }
                            }
                            ...
                            if(_db.hasRowAuthorisation(DBData.AUTHORISATION.UPDATE, "JOB", mainJobId, DBData.APPLICATION_RIGHT.MODULE_MBO, true, true)) {
                                if (!_db.Objective.isPersonFilterOnly) {
                                    ...
                                    case Objective.State.REFUSED:
                                    case Objective.State.DELETE:
                                        links.LinkGroup1.AddLink(_mapper.get("mbo", "objective"),	_mapper.get("mbo", "editObjective"), "/SPZ/Detail.aspx?view=edit&context="+_context+"&contextId="+_contextId+"&id="+_id);
                                        break;
                                    case Objective.State.ACCEPTED:
                                    case Objective.State.CONDITION:
                                        links.LinkGroup1.AddLink(_mapper.get("mbo", "objective"),	_mapper.get("mbo", "replaceObjective"), "/SPZ/Detail.aspx?view=replace&context="+_context+"&contextId="+_contextId+"&id="+_id);
                                        break;
                                    }
                                }
                                links.LinkGroup1.AddLink(_mapper.get("mbo", "objective"), _mapper.get("mbo", "editObjective"), "/SPZ/Detail.aspx?view=edit&context=" + _context + "&contextId=" + _contextId + "&id=" + _id);                       
                                ...
                                links.LinkGroup1.AddLink(_mapper.get("mbo", "objective"), _mapper.get("mbo", "copyObjectiveTo"), "/SPZ/CopyObjectiveToOE.aspx?Id=" + _id);
                            ...
                            if (_typ == Objective.ORGENTITY_TYP) {
                                ...
                                if (access) {
                                    links.LinkGroup1.AddLink(_mapper.get("mbo", "objective"),	_mapper.get("mbo", "traceObjective"), "/SPZ/Detail.aspx?view=trace&context="+_context+"&contextId="+_contextId+"&id="+_id);
                                }
                            }
                       
                            if (_typ == Objective.ORGENTITY_TYP &&
                                ((DBData.AUTHORISATION.INSERT & objectiveAccess) > 0 || (competence & 5) == 5)) {
                                string url = "/SPZ/Detail.aspx?view=copy&id="+ _id+"&context="+_context+"&contextId="+_contextId;
                                switch (state) {
                                case Objective.State.RELEASED:
                                case Objective.State.CONDITION:
                                case Objective.State.ACCEPTED:
