    ...
    public class PersonObjectiveReport : PsoftPDFReport {
        ...
        private void writeContent(string turnid) {
            ...
            foreach (DataRow r in _data.Rows) {
                ...
                // Übergeordnetes Unternehmensziel Zielart und Zielnummer hizugefügt für SPZ 2012.10.12 MSR
                val = "";
                ...
                writeProperty(table, "Übergeordnetes Unternehmensziel", val, 2, 20);
                }

                ArrayList objectiveTyp =  new ArrayList(_map.getEnum("mbo", "typ", true));
                col = _data.Columns["TYP"];
                val = _db.dbColumn.GetDisplayValue(col, r[col], false);
                val = objectiveTyp[Convert.ToInt16(val)].ToString();
                //name = _map.get("mbo", "objectivetyp");
                writeProperty(table, "Zielart", val, 2, 20);

                col = _data.Columns["NUMBER"];
                val = _db.dbColumn.GetDisplayValue(col, r[col], false);
                //name = _map.get("mbo", "number");
                writeProperty(table, "Nummer", val, 2, 20);
                ...
                writeProperty(table, "Zieltitel", val, 2, 20);

				//cst, 22.10.2007, added objective description
            ...
            // Visum
            //table.Attributes.Append(base.createAttribute("font-name", "helvetica"));
            table = base.AppendTable("4cm,3cm,6cm");
            table.Attributes.Append(base.createAttribute("keep-together","true"));
            table.Attributes.Append(base.createAttribute("padding-left","20"));
            
            //cell.Attributes.Append(base.createAttribute("font-size", 10.ToString()));
            row = table.AppendChild(base.createRow());
            cell = row.AppendChild(createCell());
            cell = row.AppendChild(createCell());
            cell.Attributes.Append(base.createAttribute("font-name", "helvetica"));
