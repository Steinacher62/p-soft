namespace ch.appl.psoft.Energiedienst.Controls
{
    ...
    public partial  class EmploymentRatingHistoryListView : PSOFTListViewUserControl
	{
		...
		{
			get {return Global.Config.baseURL + "/Energiedienst/Controls/EmploymentRatingHistoryListView.ascx";}
		}
		...
		protected override void DoExecute()
		{
			...
			try 
			{
                ...
                historyTable.Columns["IS_SELFRATING"].ExtendedProperties["Visibility"] = DBColumn.Visibility.INVISIBLE;
...
//            }
            if (row != null && cell != null)
            {
                if (cell.ClientID.IndexOf("LB_DELETE_CELL") > 0)
                {
                    ...
                    if (ch.psoft.Util.Validate.GetValid(row["LOCK"].ToString(),0) > 0 || !row["INTERVIEW_DONE"].ToString().Equals("") && !(db.isAccessorGroupMember(db.userAccessorID, groupAccessorId, true)))
                    {
                        cell.Visible = false;
                    }
                }
