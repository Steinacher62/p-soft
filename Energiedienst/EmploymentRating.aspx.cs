			...
			try
			{
				...
				switch(_mode.ToLower())
				{
					case "add":
                        ...
                        _mode = "detail";
                        Response.Redirect(base._config.baseURL+"/Energiedienst/EmploymentRating.aspx?performanceRatingID=" + _performanceRatingID + "&employmentID=" + _employmentID.ToString() + "&jobID=" + _jobID + "&mode=edit&type=" + _type.ToString());

						break;
                    case "copy":
                        DataTable otherRatings = db.getDataTable("SELECT ID, ARGUMENTS, RATING_DATE FROM PERFORMANCERATING WHERE JOB_ID = '" + _jobID + "' AND EMPLOYMENT_REF = '" + _employmentID + "' ORDER BY RATING_DATE DESC");
                        if (otherRatings.Rows.Count > 0)
                        {
                            
                                String oldID = otherRatings.Rows[0]["ID"].ToString();
                                _performanceRatingID = db.Performance.createNewPerformanceRating(_employmentID, _jobID, _type.ToLower().Equals("self"));
                                String arguments = otherRatings.Rows[0]["ARGUMENTS"].ToString();
                                db.execute("UPDATE PERFORMANCERATING SET ARGUMENTS ='" + arguments + "' WHERE ID = '" + _performanceRatingID + "'");
                                DataTable criterias = db.getDataTable("SELECT ID FROM PERFORMANCE_CRITERIA");
                                foreach (DataRow criteria in criterias.Rows)
                                {
                                    if (db.getDataTable("SELECT ID FROM PERFORMANCERATING_ITEMS WHERE PERFORMANCERATING_REF = '" + oldID + "' AND CRITERIA_REF = '" + criteria.ItemArray[0] + "'").Rows.Count > 0)
                                    {
                                    String oldArgument = db.lookup("ARGUMENTS", "PERFORMANCERATING_ARGUMENTS", "PERFORMANCERATING_REF = '" + oldID + "' AND PERFORMANCERATING_CRITERIA_REF = '" + criteria.ItemArray[0] + "'").ToString();
                                    db.execute("UPDATE PERFORMANCERATING_ARGUMENTS SET ARGUMENTS = '" + oldArgument + "' WHERE PERFORMANCERATING_REF = '" + _performanceRatingID + "' AND PERFORMANCERATING_CRITERIA_REF = '" + criteria.ItemArray[0] + "'");
                                    DataTable oldItems = db.getDataTable("SELECT ID, LEVEL_REF, RELATIV_WEIGHT FROM PERFORMANCERATING_ITEMS WHERE PERFORMANCERATING_REF = '" + oldID + "' AND CRITERIA_REF = '" + criteria.ItemArray[0] + "'");
                                    DataTable newItems = db.getDataTable("SELECT ID FROM PERFORMANCERATING_ITEMS WHERE PERFORMANCERATING_REF = '" + _performanceRatingID + "' AND CRITERIA_REF = '" + criteria.ItemArray[0] + "'");
                                    for (int i = 0; i < oldItems.Rows.Count; i++)
                                    {
                                        if (oldItems.Rows[i].ItemArray[1].ToString() != "")
                                        {
                                            db.execute("UPDATE PERFORMANCERATING_ITEMS SET LEVEL_REF = '" + oldItems.Rows[i].ItemArray[1] + "' , RELATIV_WEIGHT = '" + oldItems.Rows[i].ItemArray[2] + "' WHERE ID = '" + newItems.Rows[i].ItemArray[0] + "'");
                ...
                pageLayout.ButtonPrintAttributes.Add("onClick",
                    "javascript: window.open('" + base._config.baseURL + "/Energiedienst/PrintAveragePerformance.aspx?performanceRatingID=" + _performanceRatingID + "&employmentID=" + _employmentID + "')");
                pageLayout.ButtonPrintVisible = _mode == "edit";
                        ...
                        )
                    {
                            ...
                            && db.userId == db.Employment.getPersonId(_employmentID))
                        {
                            _links.LinkGroup1.AddLink(_mapper.get("performance", "performanceRating"),
                                ...
                                "/Energiedienst/EmploymentRating.aspx?employmentID=" + _employmentID.ToString() + "&jobID=" + _jobID + "&mode=add&type=" + _type.ToString());
                        }
                        ...
                        //        || db.hasRowAuthorisation(DBData.AUTHORISATION.INSERT, "JOB", _jobID, DBData.APPLICATION_RIGHT.PERFORMANCE_RATING, true, true))) 
                        {
                            ...
                            _links.LinkGroup1.AddLink(_mapper.get("performance", "performanceRating"),
                               ...
                               "/Energiedienst/EmploymentRating.aspx?employmentID=" + _employmentID.ToString() + "&jobID=" + _jobID + "&mode=copy&type=" + _type.ToString());
                        }
                    }
                }
                        ...
                        foreach (DataRow job in empljobs.Rows)
                        {
                                ...
                                && db.userId == db.Employment.getPersonId(_employmentID))
                            {
                                _links.LinkGroup3.AddLink(k + ". " + db.Employment.getPersonName(_employmentID) + " (" + job[db.langAttrName("JOB", "TITLE")] + ", " + db.Job.getDepartmentName((long)job["ID"]) + ")",
                                    _mapper.get("performance", "newSelfRating"),
                                    "/Energiedienst/EmploymentRating.aspx?employmentID=" + _employmentID.ToString() + "&jobID=" + job["ID"] + "&mode=add&type=" + _type.ToString());
                            }
				...
				_links.LinkGroup1.AddLink(_mapper.get("performance","performanceRating"), 
					...
					"/Energiedienst/EmploymentRating.aspx?type="+ _type +"&employmentID=" + _employmentID + "&jobID="+_jobID + "&mode=history&orderDir=desc");
				...
				switch(_mode.ToLower())
				{
					case "history":
						...
						_pHistoryList.SortURL = Global.Config.baseURL + "/Energiedienst/EmploymentRating.aspx?employmentID=" + _employmentID + "&performanceRatingID=" + _performanceRatingID + "&jobID="+_jobID +"&mode=" + _mode + "&type=" + _type;
					...
					case "edit":
                        ...
                        //_pList.SortURL = Global.Config.baseURL + "/Energiedienst/EmploymentRating.aspx?employmentID=" + _employmentID + "&performanceRatingID=" + _performanceRatingID + "&mode=" + _mode + "&type=" + _type;
                        //_pList.DetailURL = Global.Config.baseURL + "/Energiedienst/EmploymentRating.aspx?performanceRatingID=" + _performanceRatingID + "&employmentID=" + _employmentID + "&criteriaID=%CRITERIA_REF&jobexpactionsID=%EXPECTATION_REF&RatingItemID=%ID&mode=detail&type=" + _type + "&orderColumn=" + _orderColumn + "&orderDir=" + _orderDir;
                        //_pList.EditURL = Global.Config.baseURL + "/Energiedienst/EmploymentRating.aspx?performanceRatingID=" + _performanceRatingID + "&employmentID=" + _employmentID + "&criteriaID=%CRITERIA_REF&mode=edit&type=" + _type + "&orderColumn=" + _orderColumn + "&orderDir=" + _orderDir;
...
//                editing = (_locked) ? false : editing;
            ...
            */
				...
				switch(_performanceRatingID)
				{
					...
					default:
                        _links.LinkGroup1.AddLink(_mapper.get("performance","reportRating"), 
							_mapper.get("performance","performanceRatingReport"), 
                            "/Energiedienst/PrintAveragePerformance.aspx?performanceRatingID=" + _performanceRatingID + "&employmentID=" + _employmentID, "", "_new");
