    ...
    public partial class Advancement : PsoftTreeViewPage {
        ...
        protected void Page_Load(object sender, System.EventArgs e) {
            ...
            try {
                ...
                list.SortURL = Global.Config.baseURL + "/Training/Advancement.aspx?advancementID=" + list.AdvancementID + "&personID=" + list.PersonID;
                list.DetailURL = Global.Config.baseURL + "/Training/Advancement.aspx?advancementID=%ID&personID=" + list.PersonID + "&OrderColumn=" + list.OrderColumn + "&OrderDir=" + list.OrderDir;
                ...
                if (detail.PersonID > 0){
                    ...
                    {
                        long jobId = (long)db.lookup("JOB.ID","JOB INNER JOIN EMPLOYMENT ON JOB.EMPLOYMENT_ID = EMPLOYMENT.ID INNER JOIN PERSON ON EMPLOYMENT.PERSON_ID = PERSON.ID","(JOB.HAUPTFUNKTION = 1) AND (PERSON.ID = "+detail.PersonID.ToString()+")");
                        long oeId = (long)db.lookup("JOB.ORGENTITY_ID", "JOB INNER JOIN EMPLOYMENT ON JOB.EMPLOYMENT_ID = EMPLOYMENT.ID INNER JOIN PERSON ON EMPLOYMENT.PERSON_ID = PERSON.ID", "(JOB.HAUPTFUNKTION = 1) AND (PERSON.ID = " + detail.PersonID.ToString() + ")");
                        long LeaderId =db.Person.getLeader(detail.PersonID, jobId, oeId, false);
                        PsoftPageLayout.ButtonPrintAttributes.Add("onClick", "javascript: window.location.href('../report/CrystalReportViewer.aspx?alias=PersonalentwicklungsbedarfEnergiedienst&param0=" + detail.AdvancementID +"&param1="+db.Person.getWholeName(LeaderId)+"');");
                    }
                    ...
                    if (detail.hasAuthorisation(db, DBData.AUTHORISATION.INSERT)){
                        links.LinkGroup1.AddLink(_mapper.get("new"), _mapper.get(TrainingModule.LANG_SCOPE_TRAINING, TrainingModule.LANG_MENMO_CM_ADD_ADVANCEMENT), "/Training/AdvancementEdit.aspx?personID=" + detail.PersonID);
                    }
                    if (!Global.isModuleEnabled("energiedienst"))
                    {
                        if (detail.hasAuthorisation(db, DBData.AUTHORISATION.UPDATE))
                        {
                            links.LinkGroup1.AddLink(_mapper.get("actions"), _mapper.get(TrainingModule.LANG_SCOPE_TRAINING, TrainingModule.LANG_MENMO_CM_EDIT_ADVANCEMENT), "/Training/AdvancementEdit.aspx?personID=" + detail.PersonID + "&advancementID=" + detail.AdvancementID);
                        }
                    }
                    else
